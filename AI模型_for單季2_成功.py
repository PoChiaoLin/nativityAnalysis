...
# # 模型測試
# test_X = [[i] for i in range(len(quarters))]
# test_y = [float(a[quarters[i]]) for i in range(len(quarters))]
# predictions = model.predict(test_X)
# success_rate = sum([1 for i in range(len(predictions)) if predictions[i] >= lower_bound and predictions[i] <= upper_bound]) / len(predictions)
# print("模型測試成功率：{:.2%}".format(success_rate))

# # 預測下一個值
# next_quarter = "2023Q2"
# next_X = len(quarters)
# next_y = model.predict([[next_X]])[0]
# print("{}的預測值為：{:.2f}".format(next_quarter, next_y))
